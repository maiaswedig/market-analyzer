...
function cacheKey(assetId, tf, depth, target) { return `${assetId}|${tf}|${depth}|${target}`; }
...
async function okxCandles(instId, tfKey, target, onProgress) {
  ...
  if (candles.length && candles[candles.length - 1].closed === false) candles = candles.slice(0, -1);
...
/* ---------------------------------------------------------------- API pública */
/**
 * @param opts { depth:'live'|'context'|'mid'|'deep', target?:number, force?:boolean, onProgress? }
 */
export async function getCandles(asset, tfKey, opts = {}) {
  ...
  const key = cacheKey(asset.id, tfKey, depth, target);
  const ttl = ttlFor(TIMEFRAMES[tfKey].sec);
  const hit = CACHE.get(key);
  if (!opts.force && hit && Date.now() - hit.at < ttl) return { ...hit.payload, cached: true };

  const t0 = (typeof performance !== 'undefined' ? performance.now() : Date.now());
  const errors = [];
  // Se houver backend autônomo, ele é a primeira fonte. O fallback público continua disponível.
  if (!opts.skipBackend) {
    try {
      const remote = await backendGet(`/api/candles?asset=${encodeURIComponent(asset.id)}&tf=${encodeURIComponent(tfKey)}`, 2200);
      if (remote && Array.isArray(remote.candles) && remote.candles.length >= 30) {
        const payload = { candles: remote.candles, source: `Backend autônomo · ${remote.source || 'provedor configurado'}`, aggregatedFrom: null, hasVolume: !!remote.hasVolume, depth, target, latencyMs: Math.round((typeof performance !== 'undefined' ? performance.now() : Date.now()) - t0), updatedAt: remote.updatedAt || Date.now(), lastCandleTime: remote.candles[remote.candles.length-1].t, count: remote.candles.length, error: null, cached: false };
  ...
  if (asset.kind === 'crypto') {
    if (asset.okx) attempts.push(['OKX', () => okxCandles(asset.okx, tfKey, target, opts.onProgress)]);
  ...
  for (const [name, fn] of attempts) {
    try {
      ...
      // AO VIVO: a análise usa a vela que está em formação, porque o usuário
      // normalmente consulta no fim dela para projetar a próxima.
      // BACKTEST: o motor usa apenas barras fechadas (ver backtest.js), mantendo
      // causalidade sem desperdiçar a informação mais recente no modo ao vivo.
      const tfMs = TIMEFRAMES[tfKey].sec * 1000;
      const now = Date.now();
      r.candles = r.candles.filter(k => (k.t + tfMs) <= now + 1000);
  ...
  if (hit) return { ...hit.payload, cached: true, stale: true, error: 'fonte indisponível (dados em cache): ' + errors.join(' | ') };
