...
//     B0/B1 foram medidos sobre histórico real (ver tools/calibrate.mjs e LEIA-ME).
...
export const DEFAULT_SCORE_B0 = 0.06;     // |viés| abaixo disto = mercado praticamente neutro → score 50
export const DEFAULT_SCORE_B1 = 0.45;     // |viés| a partir disto = confluência máxima → score 100
 ...
 */
export function setupGrade({ score, confluence, cond, penaltyTotal, setupStats = null, blocked = false }) {
  const detail = [];
  let pts = 0;
  ...
  if (blocked) { pts -= 2; detail.push('bloqueio ativo (−2)'); }
