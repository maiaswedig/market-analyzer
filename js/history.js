...
export function addSignal(result) {
  if (!result || !result.snapshot || !result.score) return null;
  const list = loadHistory();
  const tfSec = TIMEFRAMES[result.tfKey].sec;
  // O sinal é dado com a vela atual e sempre aponta para a PRÓXIMA vela.
  const currentOpen = Math.floor(Date.now() / (tfSec * 1000)) * (tfSec * 1000);
  const predictedCandleOpen = currentOpen + tfSec * 1000;
  const isSignal = result.verdict === 'CALL' || result.verdict === 'PUT';
  if (list.some(r => r.assetId === result.asset.id && r.tf === result.tfKey && r.predictedCandleOpen === predictedCandleOpen)) return null;
  ...
  const rec = {
    ...
    predictedCandleOpen,
    predictedCandleClose: predictedCandleOpen + tfSec * 1000,
    conditions: {
...
export function historyStats(list = loadHistory(), { payout = 0.85, stake = 5 } = {}) {
  ...
  const group = (fn, src = done) => {
    ...
    return [...m.values()].map(o => {
      ...
      return { ...o, rate: o.total ? o.hits / o.total * 100 : null, ev: (p * payout - (1 - p)) * stake };
  ...
  return {
    ...
    ev: p === null ? null : (p * payout - (1 - p)) * stake,
    ...
    setupRecords: done.map(r => ({ setupId: r.setupId, setupLabel: r.setupLabel, signal: r.signal, result: r.status }))
  };
}
