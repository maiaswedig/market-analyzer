...
const state = {
  ...
  lastAlertKey: null, chartView: 'own', charts: {}, setupRanking: null,
...
numKeys.forEach(k => {
  ...
  $('#scanCount').value = String([10,15,20].includes(Number(state.settings.scannerCount)) ? state.settings.scannerCount : 20);
  $('#scanScoreMin').value = String(state.settings.scannerScoreMin || 80);
  $('#scanGrade').value = state.settings.scannerGrade || 'A+';
  ...
  const autoLearn = document.getElementById('cfgAutoLearn');
  if (autoLearn) autoLearn.checked = state.settings.autoLearn !== false;
  ...
  setInterval(() => { H.checkPending().then(n => { if (n) { renderHistory(); refreshSetupRanking(); } }); }, 10000);
  H.checkPending().then(n => { if (n) renderHistory(); });
...
function bindEvents() {
  ...
  $('#scanMarket').addEventListener('change', () => { state.settings.scannerMarket = $('#scanMarket').value; saveSettings(); updateScanNote(); });
  $('#scanCount').addEventListener('change', () => { state.settings.scannerCount = Number($('#scanCount').value); saveSettings(); updateScanNote(); });
  ...
  $('#cfgReset').addEventListener('click', () => {
    ...
    saveSettings(); renderConfig(); toast('Configurações restauradas.', 'info'); analyze();
  });
...
function applySavedLearning(asset, tf) {
  ...
  if (!item) return null;
  if (item.setupRanking) state.setupRanking = item.setupRanking;
  return item;
}
function startLearningLoop() {
  if (state.learningTimer) clearInterval(state.learningTimer);
  // O navegador precisa permanecer aberto para executar JavaScript e buscar novos candles.
  // O ciclo é leve: um ativo por rodada, girando pelo TOP 20 e persistindo os modelos no navegador.
  state.learningTimer = setInterval(() => {
    if (!state.busy && state.settings.autoLearn !== false) autoLearnCycle();
  }, 45 * 1000);
}
let learningCursor = 0;
async function autoLearnCycle() {
  if (state.busy || state.settings.autoLearn === false) return;
  const market = state.settings.scannerMarket || 'Cripto';
  const wanted = [10,15,20].includes(Number(state.settings.scannerCount)) ? Number(state.settings.scannerCount) : 20;
  const tfs = state.settings.scannerTfs?.length ? state.settings.scannerTfs : [state.tf];
  const list = universe(market).slice(0, wanted);
  if (!list.length) return;
  const asset = list[learningCursor % list.length];
  const tf = tfs[learningCursor % tfs.length];
  learningCursor++;
  await autoLearnFor(asset, tf, false);
}
async function autoLearn(force = false) {
  return autoLearnFor(state.asset, state.tf, force);
}
async function autoLearnFor(asset, tf, force = false) {
  if (state.busy || state.settings.autoLearn === false) return;
  const key = learningKey(asset, tf);
  const all = loadLearning(), old = all[key];
  if (!force && old && Date.now() - old.trainedAt < 4 * 60 * 60 * 1000) return old;
  const prevBusy = state.busy; state.busy = true;
  try {
    const settings = Object.assign({}, state.settings, { minScore: 80, minConfluence: 3, minSamples: 60, minSetupSamples: 40, highProbabilityMode: true });
    const res = await runBacktest(asset, tf, settings, { maxTests: 1600, model: null, onProgress: () => {} });
    const trainData = await getCandles(asset, tf, { depth: 'deep', target: Math.max(6000, Number(settings.deepCandles) || 6000) });
    const pool = buildSnapshotPool(trainData.candles, trainData.hasVolume, { max: 5000 }).snaps;
    const samples = pool.filter(x => x.nextDir !== null && x.nextDir !== undefined).map(x => ({ vector: x.vector, label: x.nextDir > 0 ? 1 : 0 }));
    let model = null;
    if (samples.length >= 400) {
      model = await trainLogistic(samples, { minValid: 180, epochs: 350, onProgress: () => {} });
      if (model && model.ok) { saveModel(modelKey(asset, tf), model); state.models[modelKey(asset, tf)] = model; }
    }
    const recordCount = res.signals.filter(x => x.result === 'ACERTO' || x.result === 'ERRO').length;
    const item = { trainedAt: Date.now(), asset: asset.id, tf, tests: res.bars.length, resolved: recordCount, candles: trainData.candles.length, source: trainData.source, setupRanking: res.setupRanking || null, model: model && model.ok ? { samples: model.samples, validN: model.validMetrics?.n || 0, usable: !!model.usable } : null };
    all[key] = item; saveLearning(all);
    if (asset.id === state.asset.id && tf === state.tf) {
      state.backtest = res; state.setupRanking = res.setupRanking || state.setupRanking;
      refreshSetupRanking(); renderAutoLearningStatus(item);
      loadModelFor(asset, tf);
    }
    return item;
  } catch (e) {
...
function renderAutoLearningStatus(item, error = '') {
  ...
  el.textContent = `aprendizado ${item.asset} ${item.tf} · ${item.tests || 0} testes · ${item.resolved || 0} sinais resolvidos · ${when}`;
}
...
function refreshSetupRanking(notify = false) {
  const stats = H.historyStats(H.loadHistory(), { payout: (state.settings.payout || 85) / 100, stake: state.settings.stake || 5 });
  const records = stats.setupRecords.slice();
  if (state.backtest && state.backtest.signals) {
    for (const s of state.backtest.signals) if (s.result && s.result !== 'NEUTRO') records.push({ setupId: s.setupId, setupLabel: s.setupLabel, signal: s.signal, result: s.result });
  }
  state.setupRanking = records.length ? rankSetups(records, { minSamples: state.settings.minSetupSamples, payout: (state.settings.payout || 85) / 100 }) : null;
  renderSetupPanel(records.length);
  if (notify) toast(state.setupRanking ? `Ranking atualizado com ${records.length} sinais resolvidos.` : 'Ainda não há sinais resolvidos para aprender.', 'info');
}
...
async function analyze(force = false) {
  ...
  try {
    const r = await analyzeAsset(state.asset, state.tf, state.settings, {
      model, setupRanking: state.setupRanking,
...
function renderSources(r) {
  ...
  if (main) {
    ...
    $('#srcMeta').textContent = `${main.count} candles · latência ${main.latencyMs} ms · atualizado ${fmtTime(main.updatedAt)}`;
...
async function runScanner() {
  if (state.busy) { toast('Aguarde a análise atual terminar.', 'warn'); return; }
  const market = $('#scanMarket').value;
  ...
  for (let i = 0; i < jobs.length; i++) {
    ...
    try {
      const r = await analyzeAsset(asset, tf, state.settings, { light: true, model: state.models[modelKey(asset, tf)] || null, setupRanking: state.setupRanking });
      if (!r.dataError) { const rec = H.addSignal(r); if (rec) { buildHistoryAssetFilter(); } }
...
function renderBacktest(res) {
  ...
  $('#btResults').innerHTML = `
    ...
    <div class="kpis" style="margin-top:12px">
      ...
      <div class="kpi"><span>Equilíbrio</span><b>${fmtPct(s.breakEven)}</b></div>
    ...
    <p class="small muted">Curva da banca em R$. Aposta fixa de R$ ${fmt(s.stake, 2)} — <strong>sem martingale</strong>. Uma taxa de acerto de 75% com payout ${fmt(s.payout * 100, 0)}% dá EV de R$ ${fmt(expectancy(0.75, s.payout, s.stake), 3)} por operação; abaixo de ${fmtPct(s.breakEven)} de acerto o resultado é negativo por definição.</p>

    <h2 style="margin-top:14px">Varredura de limiares</h2>
    <p class="small muted">Objetivo do <strong>MELHOR EQUILÍBRIO</strong>: ${escapeHtml(sw.objective)}.</p>
...
async function train() {
  ...
  try {
    ...
    const model = await trainLogistic(samples, { onProgress: p => { $('#mlProgress').textContent = `treinando… ${Math.round(p * 100)}%`; } });
    ...
    state.models[key] = model; saveModel(key, model);
    $('#mlProgress').textContent = 'treino concluído';
    renderMlPanel(model);
    toast(model.usable ? `Modelo validado: Brier ${model.validMetrics.brier.toFixed(4)} (taxa base ${model.baseBrier.toFixed(4)}).` : `Modelo treinado mas NÃO validado: ${model.gateReason}`, model.usable ? 'ok' : 'warn', 8000);
    analyze();
  } catch (e) {
...
function renderSetupPanel(recordCount) {
  const host = $('#setupPanel');
  const r = state.setupRanking;
  ...
  if (!r || !r.all.length) { host.innerHTML = '<p class="muted small">Sem sinais resolvidos suficientes. Rode um backtest ou acumule histórico verificado.</p>'; return; }
  ...
  host.innerHTML = block('Melhores setups', r.best, 'pos') + block('Piores setups', r.worst, 'neg') +
    ...
    `<p class="small muted">${r.classes} classes observadas; só as que atingem a amostra mínima entram no ranking. Quando a situação atual bate com uma dessas classes, ela é usada como estimativa de probabilidade na decisão (e a origem aparece na análise).</p>`;
}
...
function renderHistory() {
  ...
  const st = H.historyStats(list, { payout: (state.settings.payout || 85) / 100, stake: state.settings.stake || 5 });
