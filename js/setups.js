 ...
 */
export function fingerprint(snap, mtf, dir) {
  const parts = [];
  ...
  return { id: `${dir > 0 ? 'CALL' : 'PUT'}|${label}`, label, dir, parts };
}

/**
 * Agrega resultados por classe de setup.
 * @param records [{setupId, setupLabel, signal, result:'ACERTO'|'ERRO'|'NEUTRO'}]
 * @param opts { minSamples, payout }
 */
export function rankSetups(records, { minSamples = 20, payout = 0.85 } = {}) {
  const map = new Map();
  for (const r of records) {
    if (!r.setupId || r.result === 'NEUTRO' || !r.result) continue;
    if (!map.has(r.setupId)) map.set(r.setupId, { id: r.setupId, label: r.setupLabel || r.setupId, signal: r.signal, total: 0, hits: 0 });
    const o = map.get(r.setupId);
    o.total++; if (r.result === 'ACERTO') o.hits++;
  }
  const all = [...map.values()].map(o => {
    const rate = o.total ? o.hits / o.total * 100 : null;
    const p = o.total ? (o.hits + 1) / (o.total + 2) : null;   // Laplace
    const ev = p === null ? null : p * payout - (1 - p);
    return { ...o, rate, p, ev, enough: o.total >= minSamples };
  });
